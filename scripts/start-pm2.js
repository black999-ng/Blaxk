...
function streamLogsViaBus() {
  pm2.launchBus((busErr, bus) => {
    if (busErr) {
      const child = spawn('npx', ['pm2', 'logs', 'Blaxk_TCF'], { stdio: 'inherit', shell: true });
      child.on('exit', (code) => process.exit(code ?? 0));
      return;
    }
    console.log('📜 Streaming logs');
    bus.on('log:out', (packet) => {
      if (packet.process.name === 'Blaxk_TCF') process.stdout.write(packet.data);
    });
    bus.on('log:err', (packet) => {
      if (packet.process.name === 'Blaxk_TCF') process.stderr.write(packet.data);
    });
  });
}
...
function start() {
  ...
  pm2.connect((err) => {
    ...
    console.log('🚀 Starting Blaxk_TCF with PM2 (env=production)...');
    ...
    pm2.list((listErr, list) => {
      ...
      const exists = Array.isArray(list) && list.some((p) => p.name === 'Blaxk_TCF');
