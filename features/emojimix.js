...
async function mixEmojisToSticker(sock, msg, emoji1, emoji2) {
  ...
  const stickerBuffer = await createStickerBuffer(imageBuffer, 'Blaxk_TCF', 'Blaxk');
...
function registerEmojimixCommand({ registerCommand }) {
  ...
  const handler = async (sock, msg, args) => {
    try {
      ...
      if (!e1 || !e2) {
        await sock.sendMessage(msg.key?.remoteJid || msg.key, { text: usage(prefix) });
        return;
      }
      await mixEmojisToSticker(sock, msg, e1, e2);
    } catch (error) {
      await sock.sendMessage(msg.key?.remoteJid || msg.key, { text: `❌ Failed to mix emojis.\n\n${usage(prefix)}` });
    }
  };
