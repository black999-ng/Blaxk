...
const config = {
    ...
    botName: process.env.BOT_NAME || 'Blaxk_TCF',
...
${config.botMode === 'private' ? '🔒 Private Mode - Owner Only' : '🌐 Public Mode - Everyone'}`;
    ...
    registerCommand('sticker', 'Convert replied image to sticker or GIF for short videos', async (sock, msg) => {
        ...
        try {
            ...
            if (q.imageMessage) {
            const buffer = await downloadMediaMessage({ message: quotedMsg }, 'buffer', {}, { logger: pino({ level: 'silent' }) });
            const stickerBuffer = await createStickerBuffer(buffer, 'Blaxk_TCF', 'Blaxk');
...
💡 Use ${config.prefix}help <command> for details`;
    ...
    registerCommand('song', 'Search and download songs from YouTube', async (sock, msg, args) => {
        ...
        try {
            ...
            console.log(`✅ 🎵 Song Session Stored - Key: ${storageKeySong}, Results Count: ${results.length}`);

            // Format and send results (top 5)
            const resultsText = songs.formatSearchResults(results, query);
            
            // Send results with timer message and auto-delete
            const searchMsg = await sock.sendMessage(msg.key.remoteJid, { 
                text: resultsText + `\n\n⏱️ *Reply within 3 minutes or the list will expire*` 
            });
            
            // Auto-delete the search results message after 3 minutes
            setTimeout(async () => {
                try {
                    await sock.sendMessage(msg.key.remoteJid, { delete: searchMsg.key });
                    console.log(`🎵 Search message auto-deleted: ${storageKeySong}`);
                } catch (e) {
                    console.error('Failed to auto-delete search message:', e.message);
                }
            }, 3 * 60 * 1000);
    ...
    registerCommand('repo', 'Show bot repository link and creator info', async (sock, msg) => {
        const text = `📦 *Blaxk_TCF Repository*\n\n` +
                     `🔗 https://github.com/Blaxk/Blaxk_TCF.git\n\n` +
                     `👤 Made by *Blaxk*\n` +
    ...
    registerCommand('img', 'Generate image from text using Gemini AI', async (sock, msg, args) => {
        ...
        if (!prompt) { 
            await sock.sendMessage(jid, { text: `💡 Usage: ${config.prefix}img <prompt>\n\nExamples:\n${config.prefix}img a futuristic city skyline at night\n${config.prefix}img a cute anime girl\n${config.prefix}img a beautiful sunset over mountains` }); 
            return; 
        }
        if (!process.env.GEMINI_API_KEY) { 
            await sock.sendMessage(jid, { text: `❌ Gemini API not configured. Use ${config.prefix}setvar gemini <API_KEY>` }); 
            return; 
        }
        
        await sock.sendMessage(jid, { text: `🎨 *Generating image with Gemini...*\n\nPrompt: "${prompt}"\n\n⏳ Please wait, this may take 15-30 seconds...` });
        try { await sock.sendPresenceUpdate('composing', jid); } catch {}
        
        const res = await gemini.generateImage(prompt, 1);
        try { await sock.sendPresenceUpdate('paused', jid); } catch {}
        
        if (res.success && res.images && res.images.length) {
            for (let i = 0; i < res.images.length; i++) {
                const caption = `🖼️ *Generated Image*\n\n📝 Prompt: "${prompt}"\n\n✨ Powered by Gemini AI`;
                await sock.sendMessage(jid, { image: res.images[i], caption });
                if (i < res.images.length - 1) { await new Promise(r => setTimeout(r, 500)); }
            }
        } else {
            await sock.sendMessage(jid, { text: `❌ ${res.error || 'Image generation failed. Try a different prompt.'}` });
        }
    });
    ...
    sock.ev.on('messages.upsert', async (m) => {
        try {
            ...
            // Define userId and chatId at the start
            const userId = msg.key.participant || msg.key.remoteJid;
            const chatId = msg.key.remoteJid;
            ...
            try {
                ...
                if (sess && stickerMsg) {
                    ...
                    if (sess.items.length >= 2) {
                        ...
                        const { createStickerBuffer } = require('./features/sticker');
                        const stickerBuf = await createStickerBuffer(mixed, 'Blaxk_TCF', 'Blaxk');
            ...
            // Song feature is disabled - check for video download reply instead
            // userId and chatId already defined above
            
            // Only check for video replies if message does NOT start with prefix
...
async function showMenu() {
    console.log('\n╔════════════════════════════════════╗');
    console.log('║   🤖 Blaxk_TCF WhatsApp Bot Setup   ║');
    console.log('╚════════════════════════════════════╝\n');
...
// Start the bot
console.log('\n🚀 Starting Blaxk_TCF WhatsApp Bot...');
